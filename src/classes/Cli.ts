...
class Cli {
  ...
  createTruck(): void {
    inquirer
      ...
      .then((answers) => {
        // Create an array of 4 new Wheel objects
        const wheels: Wheel[] = Array(4).fill(new Wheel()); // Create an array of Wheel objects
        ...
        const truck = new Truck(
          ...
          wheels // Pass the wheels array
        );
  
        // Push the truck to the vehicles array
        this.vehicles.push(truck);
        this.selectedVehicleVin = truck.vin; // Set selected VIN
      ...
      .prompt([
        {
          ...
          choices: this.vehicles.filter(vehicle => vehicle instanceof Car || vehicle instanceof Motorbike).map(vehicle => ({
            name: `${vehicle.vin} -- ${vehicle.make} ${vehicle.model}`,
            value: vehicle,
          })),
        },
